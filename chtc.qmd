...
- Concatenate all the results files: `head -n +1 ./results/results_1.csv > batch_results.csv; ls ./results/results_*.csv | xargs awk 'FNR>1' >> batch_results.csv`
...
-   `find -maxdepth 1 -name "results_*.csv" | wc -l` will find results\_\*.csv in local folder (maxdepth 1) and pipe them into a word count that counts lines (-l)
