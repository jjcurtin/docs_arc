...
* `condor_history [NetID] -limit 10 -af requestmemory memoryusage` is an example of reviewing recent job history (past 10 in this example) for a subset of the history parameters.  Can list all the parameters for a single job using `condor_history 16892169.1 -l` where the jobid is listed explicitly.
